...
export default function LoginPage() {
  ...
  const { tenants, selectedTenant, loading: tenantsLoading, error: tenantsError } = useAppSelector(
    (state) => state.tenant
  );
  const { isAuthenticated, loading: authLoading, error: authError } = useAppSelector(selectAuth);
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {formik.touched.company_token && formik.errors.company_token && (
              <div className="text-sm text-red-500 mt-1">{formik.errors.company_token}</div>
            )}
          </div>

          <button
            type="submit"
            disabled={!selectedTenant || tenantsLoading || authLoading || !formik.isValid}
